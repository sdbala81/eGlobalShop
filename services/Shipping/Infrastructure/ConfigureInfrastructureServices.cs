...
namespace ElementLogiq.eGlobalShop.Shipping.Infrastructure;
...
public static class ConfigureInfrastructureServices
{
    ...
    private static IServiceCollection AddDatabase(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddDbContext<ShipmentDbContext>(optionsBuilder => optionsBuilder.UseNpgsql(
                connectionString,
                npgsqlOptions => npgsqlOptions.MigrationsHistoryTable(HistoryRepository.DefaultTableName, Schemas.DEFAULT))
            .UseSnakeCaseNamingConvention());

        services.AddScoped<IShipmentDbContext>(sp => sp.GetRequiredService<ShipmentDbContext>());
